...
class Kuka:
  ...
  def reset(self):
    #pdb.set_trace()
    objects = p.loadSDF("/home/josebarreiros/pybullet-gym/pybulletgym/envs/kuka_reach/models/kuka.sdf")
  ...
  def getObservation(self):
    ...
    for joint in range(7):
      joint_state=p.getJointState(self.kukaUid,joint)
      positions.append(joint_state[0])
      velocities.append(joint_state[1])

    iiwa_state=np.concatenate((positions,velocities))

    if self.debug:
      print("iiwa state: {}".format(iiwa_state))
    observation.extend(list(iiwa_state))
